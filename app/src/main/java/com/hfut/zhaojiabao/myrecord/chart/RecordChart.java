...
public class RecordChart extends View implements BaseChart {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int min = (int) (mDatas.size() - (1.5 * mPerWidth + mWidth / 2 + mExcursion) / mPerWidth - 1);
        int max = (int) ((2.5 * mPerWidth + mWidth / 2 - mExcursion) / mPerWidth + mDatas.size() - 1);
        ...
        for (int i = min; i <= max; i++) {
            float left = mWidth / 2 - mPerWidth / 2 - mPerWidth * (mDatas.size() - i - 1) + mExcursion;
            float right = left + mPerWidth;
            float top = mHeight - mDatas.get(i).value * mHeightPercent - mXHeight;
            float bottom = mHeight - mXHeight;
